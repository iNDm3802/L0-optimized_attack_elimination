...
def mean(lst):
    ...
    for item in lst:
        res += item
    res = np.divide(res, len(lst), casting='unsafe')

    return res


def l2(list_1, list_2, mode='c'):
    if mode == 'c':
...
def restore_pixel(restore_mode, img, x, y, mode='c'):
    if restore_mode == 'bilinear':
        restored_pixel = interpolate.bilinear(img=img, x=x, y=y, mode=mode)
    elif restore_mode == 'bicubic':
        restored_pixel = interpolate.bicubic(img=img, x=x, y=y, mode=mode)
    elif restore_mode == 'lanczos':
        restored_pixel = interpolate.lanczos(img=img, x=x, y=y, mode=mode)
    elif restore_mode == 'telea':
        restored_pixel = inpaint.telea(img=img, x=x, y=y, mode=mode)
    elif restore_mode == 'navier_stokes':
        restored_pixel = inpaint.navier_stokes(img=img, x=x, y=y, mode=mode)
    elif restore_mode == 'neighbors':
        restored_pixel = other.neighbors(img=img, x=x, y=y, mode=mode)
    elif restore_mode == 'gauss_blur':
        restored_pixel = other.gauss_blur(img=img, x=x, y=y, mode=mode)
    else:
...
def restore_image(restore_mode, img, pixels, save=True, save_path="", compute_metrics=True,
    ...
    for pixel in pixels:
        ...
        if compute_metrics:
            true_pixel = pixel['cln_color']
            dist = l2(true_pixel, restored_pixel, mode=mode)
...
#
#         if compute_metrics:
#             dist = l2(true_pixel, restored_pixel, mode)
#             distances.append(dist)
#
#             cos = angle(true_pixel, restored_pixel)
#             metric = dist / cos
...
# mode = 'c'
# path = "CIFAR10/1px/"
# path = "CIFAR10/jsma 001/"
# path = "CIFAR10/jsma 003/"
# path = "CIFAR10/jsma 005/"
# model = to_device(Cifar10CnnModel(), device)
# model.load_state_dict(torch.load('models/cifar10-cnn.pth'))
mode = 'g'
# path = "CIFAR10G/1px/"
# path = "CIFAR10G/jsma 001/"
# path = "CIFAR10G/jsma 003/"
path = "CIFAR10G/jsma 005/"
...
if mode != 'c':
    mode = 'g'
if 'jsma' in path:
    stat = parse_attack_stat_jsma(path + "!result.txt", mode=mode)
else:
    stat = parse_attack_stat_1px(path + "!result.txt", mode=mode)
...
# output = open(f'!{mode_name}_' + path.split('/')[1] + '.txt', 'w')
# output.write(header + '\n')
# output.close()
# print(header)
#
# time_header = "method\ttime"
# output_time = open(f'!{mode_name}_' + path.split('/')[1] + '_time.txt', 'w')
...
#             restored_pixel = restore_pixel(restore_mode=restore_mode, img=img, x=x, y=y, mode=mode)
...
#     output = open(f'!{mode_name}_' + path.split('/')[1] + '.txt', 'a')
#     output.write(line + '\n')
#     output.close()
#     print(line)
#
#     time_line = f"{restore_mode}\t{time_compute}"
#     output_time = open(f'!{mode_name}_' + path.split('/')[1] + '_time.txt', 'a')
...
output = open(f'!{mode_name}_' + path.split('/')[1] + '_fix' + '.txt', 'a')
# output = open(f'!{mode_name}_' + path.split('/')[1] + '_fix_full' + '.txt', 'a')
